...
double simulate() {
 ...
 // while ( ( zah_factor > 1.0 && zAH < zah_final ) ||
  //      ( zah_factor < 1.0 && zAH > zah_final ) ) {
    ...
    complex<double> polyterm();
    complex<double> npterm();
    complex<double> dUp;
    complex<double> dUnp;
    dUp = -1.0*polyterm();
    dUnp = npterm();
    ...
    TS = real(H) - dU;
    if ( myrank == 0 ) {
      fprintf(zot, "muA muB p H U S npH pH\n");
      fprintf(zot, "%lf %lf %lf %lf %lf %lf %lf %lf\n", muA, muB, real(H)/V, real(H),real(dU),real(TS),real(dUnp),real(dUp));
      fflush(zot) ;
    }
 // zAH *= zah_factor ;
...
complex<double> term1() {
      int i;
      for ( i=0; i<ML ; i++ )
        tmp[i] = (wabp[i] * wabp[i] + wabm[i] * wabm[i] ) * C/chiABN + exp_nr_chiAPN*(rho_exp_nr[i]*rhoha[i]) + exp_nr_chiBPN*(rho_exp_nr[i]*rhodb[i]);
...
complex<double> npterm() {
      ...
      int i;
      for ( i=0; i<ML ; i++ )
        tmp[i] = exp_nr_chiAPN*(rho_exp_nr[i]*rhoha[i]) + exp_nr_chiBPN*(rho_exp_nr[i]*rhodb[i]);
      return integ_trapPBC( tmp );
}
